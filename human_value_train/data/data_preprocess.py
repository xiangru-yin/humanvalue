...
def load_and_process_data(datapath):
    ...
    if all_dfs:
        ...
        # 确保列的顺序一致
        combined_df = combined_df[required_columns].fillna("")
        combined_df = combined_df[combined_df["大模型汇总价值观"]!=""].reset_index(drop=True)
...
if __name__ == "__main__":
    ...
    # 查看结果
    print("\n处理后的数据集示例（包含缺失字段）:")
    training_data_output_path = "../dataset/wuhan_train_vt_only_v10.parquet"
