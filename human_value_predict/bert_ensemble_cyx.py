...
import humanvalue_api as human_value_api
from humanvalue_api.model.IntergrateBERT_predict import DebertaMultiLabelClassifier
from humanvalue_api.DataModule import BertDataset
...
def init(checkpoint_path="humanvalue_api/checkpoints/last-0.796.ckpt",
         batch_size=2,
         max_token_count=512,
         num_workers=0,
         device=None,
         model_name='saved_models/IDEA-CCNL/Erlangshen-DeBERTa-v2-320M-Chinese',
...
def forward_file(event_data_csv_path="",
    ...
    for idx, pred in enumerate(predictions):
        ...
        for batch_idx in range(predicted_classes.shape[0]):
            ...
            for label_idx in range(_params["NUM_LABELS"]):
                ...
                ensemble_value_dict[f'{label_name}'] = multi_label # 更换为多分类标签
    ...
    #post_df.to_csv(post_result_csv_path, index=False, encoding='utf-8-sig')
...
if __name__ == "__main__":
    init(
        checkpoint_path="../human_value_predict/humanvalue_api/checkpoints/last-0.8.ckpt",
        batch_size=8,
        max_token_count=512
    )
    get_value_ensemble_dir_bert(
        "../human_value_predict/human_value_data/valuetalk_nof_bert_multi_result"
    )
