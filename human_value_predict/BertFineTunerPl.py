...
class BertFineTunerPl(pl.LightningModule):
    ...
    def __init__(self, n_classes: int, params, label_columns, n_training_steps=None, n_warmup_steps=None):
        super().__init__()
        self.bert = AutoModel.from_pretrained(f"..{os.sep}human_value_predict{os.sep}saved_models{os.sep}danschr{os.sep}roberta-large-BS_16-EPOCHS_5-LR_5e-05-ACC_GRAD_2-MAX_LENGTH_165", return_dict=True, mirror='https://hf-mirror.com')
...
class BertFineTunerPl(pl.LightningModule):
    def __init__(self, params, label_columns, n_classes):
        super().__init__()
        # 直接加载原始 RoBERTa 结构（与检查点一致）
